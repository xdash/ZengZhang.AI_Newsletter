...
> 原链: https://zj1123581321.com/posts/260809-multi-agent-scheduling-architecture/  
> 来源: zj1123581321.com · 归档自 EP.69 · 抓取于 2026-08-27  

---

![将军赶路不追小兔](images/agent%e6%8e%a7%e5%88%b6%e5%8f%b0.png)


这篇的底稿是一段 80 分钟的语音分享。依然延续这个系列的传统——**作为人类，你只需要读懂思路和架构逻辑；所有具体的执行细节，把文章丢给你的 Claude Code 或 Codex，让它去做就行。**


## 开头：为什么要聊调度体系[#](#开头为什么要聊调度体系)
...
```
### 第一层：单 Agent 内部的"主脑 + 子代理"[#](#第一层单-agent-内部的主脑--子代理)
...
### 第二层：产品化的多 Agent 编排（推荐大多数人从这里开始）[#](#第二层产品化的多-agent-编排推荐大多数人从这里开始)
...
### 第三层：自定义命令行编排（我的方案）[#](#第三层自定义命令行编排我的方案)
...
![herdr 界面：三个工作区、四个 Agent，两个在干活、两个在等待——没有人盯着，这就是重点（图源：herdr.dev）](images/image.png)
...
## 三、质量保障：让 Agent 自己管住自己[#](#三质量保障让-agent-自己管住自己)
...
## 四、让 Agent 自主进入生产环境[#](#四让-agent-自主进入生产环境)
...
每周我都会针对过去的开发记录，调最好的模型做一次复盘。复盘的核心依据是本地保存的对话日志和轨迹数据（下一章会详细讲为什么要保留这些数据、以及该怎么保留）。会结合 Claude Code 和 Codex 的官方最佳实践，看当前和 Agent 的协作过程中还有哪些卡点、可以提升的地方。
...
![我的 Token 用量监控面板：近 30 天模型分布与协作概览](images/%e8%bf%87%e5%8e%bb%2030%20%e5%a4%a9%e6%a8%a1%e5%9e%8b%e5%88%86%e5%b8%83.png)


### 1. 每日 Token 消耗（含缓存）[#](#1-每日-token-消耗含缓存)
...
### 4. 保留一切轨迹数据[#](#4-保留一切轨迹数据)

前面聊周期性复盘时提到，复盘的核心依据是本地的对话日志和轨迹数据。**没有记录就没办法复盘，没有复盘就没办法提升。**
...
![Turns 趋势与效率指标面板](images/%e6%8c%87%e6%a0%87%e7%9b%91%e6%b5%8b.png)


### 5. 每百万 Token 的任务执行成本[#](#5-每百万-token-的任务执行成本)
...
### 7. 每次提交都能追溯到"谁干的"[#](#7-每次提交都能追溯到谁干的)
...
## 六、上下文管理：最容易被忽视的核心战场[#](#六上下文管理最容易被忽视的核心战场)
...
我个人最喜欢的区间是**对话窗口的前 20%**——这个范围内 Agent 的思维质量通常是最好的。当然实际使用中不可能每次都这么克制，偶尔也会用到六七成。随着模型和工具都在进化，这个比例不是放之四海而皆准的定律。关键是培养一种直觉：知道什么时候该把当前对话里的关键信息总结成一份文档，然后开一个全新的对话让 Agent “满血复活”。

还有一条原则：**所有低成本的记录方式，都应该去做。** 管它当下有没有用呢？让 Agent 结合你的业务场景，判断应该记录哪些日志、保留哪些上下文，方便它之后定位分析。数字世界里的记录成本几乎为零，但收益经常在你意想不到的时候冒出来。

## 七、行业参考：不是只有我在做这件事[#](#七行业参考不是只有我在做这件事)

### “别做 Loop，建软件工厂”[#](#别做-loop建软件工厂)
...
### 每个 Loop 都成功，系统为什么还在变坏？[#](#每个-loop-都成功系统为什么还在变坏)

[一篇来自 Castbox Guru 团队（基于 [Trellis](https://docs.trytrellis.app/) 框架做 AI 驱动研发）的深度文章]([https://mp.weixin.qq.com/s/CiV0mZwwB6ZT-CEHbpBq3Q](https://mp.weixin.qq.com/s/CiV0mZwwB6ZT-CEHbpBq3Q))诊断了一个深层问题：
...
### Vibe Coding 做大项目一定会塌掉，除非……[#](#vibe-coding-做大项目一定会塌掉除非)

[徐文浩在 AI 炼金术的分享](https://mp.weixin.qq.com/s/kpeoDfe87-PY5yXpOm00fA)给了我很多工程实践上的启发：

纯 vibe coding（凭感觉让 AI 写代码）做大项目一定会塌掉。不塌只有一条路——搭并持续迭代你的工具和流程体系。


我在搭自己这套体系之前就有一些基础判断——比如门禁不能省、写和审必须分开。但他的团队在落地层面走得更深：双重代码审查、几十道质量门禁、自动化测试加录屏、定期全库架构审查、自动修复线上报错。我从中学到了很多具体的工程实践思路，差异主要在规模（他是团队级别，我是个人）和具体工具选型上。
...
## 附录：给 Agent 读的参考材料[#](#附录给-agent-读的参考材料)
...
本文由飞书录音豆语音转文字构思底稿，通过 Claude Code + Opus 4.6 完成文章架构设计与内容整理。题图基于 GPT Image-2 生成。
